...
import {
	SpoolTemplateGetParamsType,
	SpoolTemplateType,
	SpoolTemplateGetParams,
	SpoolTemplateCreateInputType,
	SpoolTemplateCreateInput,
	SpoolTemplatePutInputType,
	SpoolTemplatePutInput
} from '../schemas/SpoolTemplate';
...
const SpoolTemplatesRoute: Route = async (fastify, ctx) => {
	...
	fastify.get<{
		Params: SpoolTemplateGetParamsType;
		Reply: SpoolTemplateType;
	}>(
		'/spool-templates/:id',
		{
			schema: {
		...
		(req, resp) => {
			return auth(req, resp, ctx, async user => {
				const spoolTemplate = await ctx.db.conn
					.getRepository(SpoolTemplate)
					.findOne(req.params.id);

				if (
					!spoolTemplate ||
					!(await ctx.authz.authorize(user, 'read', spoolTemplate))
				) {
	...
	fastify.post<{ Body: SpoolTemplateCreateInputType }>(
		'/spool-templates',
		{
			schema: {
	...
	fastify.put<{
		Params: SpoolTemplateGetParamsType;
		Body: SpoolTemplatePutInputType;
	}>(
		'/spool-templates/:id',
		{
			schema: {
		...
		(req, resp) => {
			return auth(req, resp, ctx, async user => {
				...
				if (
					!spoolTemplate ||
					!(await ctx.authz.authorize(user, 'modify', spoolTemplate))
				) {
					forbiddenHelper(req, resp);
					return;
				}
	...
	fastify.delete<{ Params: SpoolTemplateGetParamsType }>(
		'/spool-templates/:id',
		{
			schema: {
				params: SpoolTemplateGetParams
		...
		(req, resp) => {
			return auth(req, resp, ctx, async user => {
				...
				if (
					!spoolTemplate ||
					!(await ctx.authz.authorize(user, 'delete', spoolTemplate))
				) {
					forbiddenHelper(req, resp);
					return;
				}
